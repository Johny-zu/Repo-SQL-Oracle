...
-- Para usar el group by
SELECT DEPARTMENT_ID, JOB_ID, SUM(SALARY) FROM EMPLOYEES
GROUP BY DEPARTMENT_ID, JOB_ID ORDER BY DEPARTMENT_ID, JOB_ID;

--Para usar havin
/*havin: Filtra los grupos resultantes para incluir solo aquellos
 donde la suma total de los salarios es mayor que 500.*/
...
-- funcion AVG
/*Promedio del salario de cada job_id usando employees*/
SELECT AVG(SALARY), JOB_ID FROM EMPLOYEES
GROUP BY JOB_ID ORDER BY JOB_ID;

-- funcoin coutn con group by
/*ndica que hay n empleados para cada uno de los puestos dads*/
SELECT COUNT(JOB_ID), JOB_ID FROM EMPLOYEES
GROUP BY JOB_ID;

-- buscar entre tablas
SELECT COUNT(EMPLOYEE_ID) "NUM.EMPLEADOS", JOB_TITLE "CARGO" FROM EMPLOYEES, JOBS
WHERE EMPLOYEES.JOB_ID = JOBS.JOB_ID GROUP JOB_TITLE;

SELECT SUM(SALARY), DEPARTMENT_NAME FROM EMPLOYEES, DEPARTMENTS
